...
import (
	...
	_ "github.com/sandwich-go/boost/xrand"
)
...
func Float32sContain(s []float32, v float32) bool {
	for _, ele := range s {
		if ele == v {
			return true
		}
...
func Float32sSetAdd(s []float32, v ...float32) []float32 {
	for _, ele := range v {
		if !Float32sContain(s, ele) {
			s = append(s, ele)
		}
	}
	return s
...
func Float32sWalk(s []float32, f func(float32) (float32, bool)) []float32 {
...
func Float32sRemoveRepeated(s []float32) []float32 {
	if len(s) == 0 {
		return s
	}
	if len(s) < tooManyElement {
		return float32RemoveRepeatByLoop(s)
	} else {
		return float32RemoveRepeatByMap(s)
	}
}

// Float32sRemoveEmpty 移除空元素
func Float32sRemoveEmpty(s []float32) []float32 {
	out := make([]float32, 0, len(s))
	for _, ele := range s {
		if ele != 0 {
			out = append(out, ele)
...
func float32RemoveRepeatByMap(s []float32) []float32 {
	out := make([]float32, 0, len(s))
...
func float32RemoveRepeatByLoop(s []float32) []float32 {
	out := make([]float32, 0, len(s))
	flag := true
	for i := range s {
		flag = true
...
func Float32sShuffle(s []float32) {
	for i := range s {
		j := rand.Intn(i + 1)
		s[i], s[j] = s[j], s[i]
	}
...
func Float64sContain(s []float64, v float64) bool {
	for _, ele := range s {
		if ele == v {
			return true
		}
...
func Float64sSetAdd(s []float64, v ...float64) []float64 {
	for _, ele := range v {
		if !Float64sContain(s, ele) {
			s = append(s, ele)
		}
	}
	return s
...
func Float64sWalk(s []float64, f func(float64) (float64, bool)) []float64 {
...
func Float64sRemoveRepeated(s []float64) []float64 {
	if len(s) == 0 {
		return s
	}
	if len(s) < tooManyElement {
		return float64RemoveRepeatByLoop(s)
	} else {
		return float64RemoveRepeatByMap(s)
	}
}

// Float64sRemoveEmpty 移除空元素
func Float64sRemoveEmpty(s []float64) []float64 {
	out := make([]float64, 0, len(s))
	for _, ele := range s {
		if ele != 0 {
			out = append(out, ele)
...
func float64RemoveRepeatByMap(s []float64) []float64 {
	out := make([]float64, 0, len(s))
...
func float64RemoveRepeatByLoop(s []float64) []float64 {
	out := make([]float64, 0, len(s))
	flag := true
	for i := range s {
		flag = true
...
func Float64sShuffle(s []float64) {
	for i := range s {
		j := rand.Intn(i + 1)
		s[i], s[j] = s[j], s[i]
	}
