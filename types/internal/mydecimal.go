...
// Original source: https://github.com/chrislusf/gleamold/blob/404dc1e31e45da114543d8c19b9f0ef4c4edc984/sql/util/types/mydecimal.go
...
const (
	...
	maxWordBufLen = 12 // A MyDecimal holds 9 words. 11*9=99位，最大支持10^99
...
func (d *MyDecimal) Copy(f *MyDecimal) {
	d.digitsInt = f.digitsInt
	d.digitsFrac = f.digitsFrac
	d.resultFrac = f.resultFrac
	d.negative = f.negative
	for i := 0; i < maxWordBufLen; i++ {
		d.wordBuf[i] = f.wordBuf[i]
	}
}
...
func (d *MyDecimal) String() string {
	tmp := *d
	boost.LogErrorAndEatError(tmp.Round(&tmp, int(tmp.resultFrac)))
...
func (d *MyDecimal) Shift(shift int) error {
	...
	if newLen > wordBufLen {
		...
		boost.LogErrorAndEatError(d.Round(d, digitEnd-point-diff))
		digitEnd -= diff
		digitsFrac = wordsFrac * digitsPerWord
...
func (d *MyDecimal) Round(to *MyDecimal, frac int) (err error) {
	...
	if d.digitsFrac <= int8(frac) {
		to.digitsInt = d.digitsInt
		to.digitsFrac = d.digitsFrac
		to.resultFrac = d.resultFrac
		to.negative = d.negative
		for i := 0; i < maxWordBufLen; i++ {
			to.wordBuf[i] = d.wordBuf[i]
		}
		return nil
	}
...
func NewDecFromFloatForTest(f float64) *MyDecimal {
	dec := new(MyDecimal)
	boost.LogErrorAndEatError(dec.FromFloat64(f))
	return dec
}
...
func NewDecFromStringForTest(s string) *MyDecimal {
	dec := new(MyDecimal)
	boost.LogErrorAndEatError(dec.FromString([]byte(s)))
	return dec
}
...
func NewMaxOrMinDec(negative bool, prec, frac int) *MyDecimal {
	...
	str[1+prec-frac] = '.'
	dec := new(MyDecimal)
	boost.LogErrorAndEatError(dec.FromString(str))
	return dec
}
