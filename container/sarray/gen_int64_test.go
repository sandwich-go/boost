...
func TestInt64(t *testing.T) {
	Convey("test sync array", t, func() {
		for _, tr := range []*Int64{NewInt64(), NewSyncInt64()} {
			...
			tr.SetArray([]int64{__formatToInt64(1), __formatToInt64(1), __formatToInt64(1), __formatToInt64(1), __formatToInt64(1)})
			So(tr.Len(), ShouldEqual, 5)
			tr.SetArray([]int64{__formatToInt64(1), __formatToInt64(2), __formatToInt64(3), __formatToInt64(4), __formatToInt64(5)})
			So(tr.Len(), ShouldEqual, 5)

			rpv := __formatToInt64(20)
			err := tr.Set(2, rpv)
			So(err, ShouldBeEmpty)
			So(tr.At(2), ShouldEqual, rpv)

			tr.Replace([]int64{__formatToInt64(1), __formatToInt64(1)})
			So(tr.Len(), ShouldEqual, 5)
			So(tr.At(0), ShouldEqual, tr.At(1))
			So(tr.At(2), ShouldNotEqual, tr.At(0))

			iv1 := __formatToInt64(11)
			err = tr.InsertBefore(0, iv1)
			So(err, ShouldBeNil)
			So(tr.At(0), ShouldEqual, iv1)

			iv2 := __formatToInt64(12)
			...
			pl := __formatToInt64(11)
			tr.PushLeft(pl)
			So(tr.At(0), ShouldEqual, pl)
			pr := __formatToInt64(21)
			tr.PushRight(pr)
			...
			aps := []int64{__formatToInt64(35), __formatToInt64(40), __formatToInt64(45), __formatToInt64(50)}
			tr.Append(aps...)
			for i := len(aps); i > 0; i-- {
				So(aps[i-1], ShouldEqual, func() int64 { v, f = tr.PopRight(); So(f, ShouldBeTrue); return v }())
			}
			...
			k := 0
			tr.WalkAsc(func(key int, val int64) bool {
			...
			k = len(s) - 1
			tr.WalkDesc(func(key int, val int64) bool {
				So(key, ShouldEqual, k)
				So(val, ShouldEqual, s[k])
				k--
				return true
			...
			So(func() {
				tr.LockFunc(func(array []int64) {
					return
				})
			}, ShouldNotPanic)

			So(func() {
				tr.RLockFunc(func(array []int64) {
					return
				})
			}, ShouldNotPanic)
