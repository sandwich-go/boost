...
func TestUint(t *testing.T) {
	Convey("test sync array", t, func() {
		for _, tr := range []*Uint{NewUint(), NewSyncUint()} {
			...
			tr.SetArray([]uint{__formatToUint(1), __formatToUint(1), __formatToUint(1), __formatToUint(1), __formatToUint(1)})
			So(tr.Len(), ShouldEqual, 5)
			tr.SetArray([]uint{__formatToUint(1), __formatToUint(2), __formatToUint(3), __formatToUint(4), __formatToUint(5)})
			So(tr.Len(), ShouldEqual, 5)

			rpv := __formatToUint(20)
			err := tr.Set(2, rpv)
			So(err, ShouldBeEmpty)
			So(tr.At(2), ShouldEqual, rpv)

			tr.Replace([]uint{__formatToUint(1), __formatToUint(1)})
			So(tr.Len(), ShouldEqual, 5)
			So(tr.At(0), ShouldEqual, tr.At(1))
			So(tr.At(2), ShouldNotEqual, tr.At(0))

			iv1 := __formatToUint(11)
			err = tr.InsertBefore(0, iv1)
			So(err, ShouldBeNil)
			So(tr.At(0), ShouldEqual, iv1)

			iv2 := __formatToUint(12)
			...
			pl := __formatToUint(11)
			tr.PushLeft(pl)
			So(tr.At(0), ShouldEqual, pl)
			pr := __formatToUint(21)
			tr.PushRight(pr)
			...
			aps := []uint{__formatToUint(35), __formatToUint(40), __formatToUint(45), __formatToUint(50)}
			tr.Append(aps...)
			for i := len(aps); i > 0; i-- {
				So(aps[i-1], ShouldEqual, func() uint { v, f = tr.PopRight(); So(f, ShouldBeTrue); return v }())
			}
			...
			k := 0
			tr.WalkAsc(func(key int, val uint) bool {
			...
			k = len(s) - 1
			tr.WalkDesc(func(key int, val uint) bool {
				So(key, ShouldEqual, k)
				So(val, ShouldEqual, s[k])
				k--
				return true
			...
			So(func() {
				tr.LockFunc(func(array []uint) {
					return
				})
			}, ShouldNotPanic)

			So(func() {
				tr.RLockFunc(func(array []uint) {
					return
				})
			}, ShouldNotPanic)
