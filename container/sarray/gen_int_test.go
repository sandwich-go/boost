...
func TestInt(t *testing.T) {
	Convey("test sync array", t, func() {
		for _, tr := range []*Int{NewInt(), NewSyncInt()} {
			...
			tr.SetArray([]int{__formatToInt(1), __formatToInt(1), __formatToInt(1), __formatToInt(1), __formatToInt(1)})
			So(tr.Len(), ShouldEqual, 5)
			tr.SetArray([]int{__formatToInt(1), __formatToInt(2), __formatToInt(3), __formatToInt(4), __formatToInt(5)})
			So(tr.Len(), ShouldEqual, 5)

			rpv := __formatToInt(20)
			err := tr.Set(2, rpv)
			So(err, ShouldBeEmpty)
			So(tr.At(2), ShouldEqual, rpv)

			tr.Replace([]int{__formatToInt(1), __formatToInt(1)})
			So(tr.Len(), ShouldEqual, 5)
			So(tr.At(0), ShouldEqual, tr.At(1))
			So(tr.At(2), ShouldNotEqual, tr.At(0))

			iv1 := __formatToInt(11)
			err = tr.InsertBefore(0, iv1)
			So(err, ShouldBeNil)
			So(tr.At(0), ShouldEqual, iv1)

			iv2 := __formatToInt(12)
			...
			aps := []int{__formatToInt(35), __formatToInt(40), __formatToInt(45), __formatToInt(50)}
			tr.Append(aps...)
			for i := len(aps); i > 0; i-- {
