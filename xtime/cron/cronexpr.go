...
func Parse(expr string) (cronExpr *Expression, err error) {
	fields := strings.Fields(expr)
	if len(fields) != 5 && len(fields) != 6 {
		err = xerror.NewText("invalid expr %v: expected 5 or 6 fields, got %v", expr, len(fields))
		return
	}
...
onError:
	err = xerror.NewText("invalid expr %v: %v", expr, err)
	return
}
...
func parseCronField(field string, min int, max int) (cronField uint64, err error) {
	...
	for _, field := range fields {
		...
		if len(rangeAndIncr) > 2 {
			err = xerror.NewText("too many slashes: %v", field)
			return
		}
		...
		if len(startAndEnd) > 2 {
			err = xerror.NewText("too many hyphens: %v", rangeAndIncr[0])
			return
		}
		...
		if startAndEnd[0] == "*" {
			if len(startAndEnd) != 1 {
				err = xerror.NewText("invalid range p1: %v", rangeAndIncr[0])
				return
			}
			start = min
			end = max
			...
			if err != nil {
				err = xerror.NewText("invalid range p2: %v", rangeAndIncr[0])
				return
			}
			// end
			if len(startAndEnd) == 1 {
				...
				if err != nil {
					err = xerror.NewText("invalid range p3: %v", rangeAndIncr[0])
					return
				}
			}
		}
		...
		if start > end {
			err = xerror.NewText("invalid range p4: %v", rangeAndIncr[0])
			return
		}
		if start < min {
			err = xerror.NewText("start out of range [%v, %v]: %v", min, max, rangeAndIncr[0])
			return
		}
		if end > max {
			err = xerror.NewText("end out of range [%v, %v]: %v", min, max, rangeAndIncr[0])
			return
		}
		...
		if len(rangeAndIncr) == 1 {
			...
			if err != nil {
				err = xerror.NewText("invalid increment: %v", rangeAndIncr[1])
				return
			}
			if incr <= 0 {
				err = xerror.NewText("invalid increment: %v", rangeAndIncr[1])
