...
var DefaultShardCountStringAny = uint64(32)

type StringAny struct {
	shardedList  []*shardedStringAny
	shardedCount uint64
}

type shardedStringAny struct {
...
func NewWithSharedCountStringAny(sharedCount uint64) *StringAny {
	p := &StringAny{
		shardedCount: sharedCount,
		shardedList:  make([]*shardedStringAny, sharedCount),
	}
	for i := uint64(0); i < sharedCount; i++ {
		p.shardedList[i] = &shardedStringAny{items: make(map[string]interface{})}
	}
	return p
}
...
func NewStringAny() *StringAny {
	return NewWithSharedCountStringAny(DefaultShardCountStringAny)
}

// GetShard 返回key对应的分片
func (m *StringAny) GetShard(key string) *shardedStringAny {
...
// Set 设定元素
func (m *StringAny) Set(key string, value interface{}) {
...
func (m *StringAny) GetAll() map[string]interface{} {
...
// })
//
func (m *StringAny) ClearWithFuncLock(onClear func(key string, val interface{})) {
	for _, shard := range m.shardedList {
		shard.Lock()
		for key, val := range shard.items {
			onClear(key, val)
...
// MGet 返回多个元素
func (m *StringAny) MGet(keys ...string) map[string]interface{} {
	data := make(map[string]interface{})
...
// MSet 同时设定多个元素
func (m *StringAny) MSet(data map[string]interface{}) {
...
func (m *StringAny) SetNX(key string, value interface{}) (isSet bool) {
	shard := m.GetShard(key)
	shard.Lock()
...
//  })
//
func (m *StringAny) LockFuncWithKey(key string, f func(shardData map[string]interface{})) {
	shard := m.GetShard(key)
	shard.Lock()
	defer shard.Unlock()
	f(shard.items)
...
//  })
//
func (m *StringAny) RLockFuncWithKey(key string, f func(shardData map[string]interface{})) {
	shard := m.GetShard(key)
	shard.RLock()
	defer shard.RUnlock()
	f(shard.items)
...
//  })
//
func (m *StringAny) LockFunc(f func(shardData map[string]interface{})) {
	for _, shard := range m.shardedList {
		shard.Lock()
		f(shard.items)
		shard.Unlock()
...
//  })
//
func (m *StringAny) RLockFunc(f func(shardData map[string]interface{})) {
	for _, shard := range m.shardedList {
		shard.RLock()
		f(shard.items)
		shard.RUnlock()
...
func (m *StringAny) doSetWithLockCheck(key string, val interface{}) (result interface{}, isSet bool) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *StringAny) doSetWithLockCheckWithFunc(key string, f func(key string) interface{}) (result interface{}, isSet bool) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *StringAny) GetOrSetFunc(key string, f func(key string) interface{}) (result interface{}, isSet bool) {
...
//  })
//
func (m *StringAny) GetOrSetFuncLock(key string, f func(key string) interface{}) (result interface{}, isSet bool) {
	if v, ok := m.Get(key); ok {
		return v, false
	}
...
func (m *StringAny) GetOrSet(key string, val interface{}) (interface{}, bool) {
	if v, ok := m.Get(key); ok {
		return v, false
	}
...
func (m *StringAny) Get(key string) (interface{}, bool) {
	shard := m.GetShard(key)
	shard.RLock()
...
// Len Count方法别名
func (m *StringAny) Len() int { return m.Count() }

// Size Count方法别名
func (m *StringAny) Size() int { return m.Count() }

// Count 返回容器内元素数量
func (m *StringAny) Count() int {
	count := 0
...
// Remove 删除key对应的元素
func (m *StringAny) Remove(key string) {
	shard := m.GetShard(key)
	shard.Lock()
...
func (m *StringAny) GetAndRemove(key string) (interface{}, bool) {
	shard := m.GetShard(key)
	shard.Lock()
	val, ok := shard.items[key]
	delete(shard.items, key)
...
//  }
//
func (m *StringAny) Iter() <-chan TupleStringAny {
	ch := make(chan TupleStringAny)
	go func() {
		// Foreach shard.
		for _, shard := range m.shardedList {
			shard.RLock()
...
func (m *StringAny) IterBuffered() <-chan TupleStringAny {
	ch := make(chan TupleStringAny, m.Count())
	go func() {
		// Foreach shard.
		for _, shard := range m.shardedList {
			// Foreach key, value pair.
