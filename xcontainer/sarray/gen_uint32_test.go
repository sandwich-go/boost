...
func TestUint32(t *testing.T) {
	Convey("test sync array", t, func() {
		for _, tr := range []*Uint32{NewUint32(), NewSyncUint32()} {
			...
			tr.SetArray([]uint32{__formatToUint32(1), __formatToUint32(1), __formatToUint32(1), __formatToUint32(1), __formatToUint32(1)})
			So(tr.Len(), ShouldEqual, 5)
			tr.SetArray([]uint32{__formatToUint32(1), __formatToUint32(2), __formatToUint32(3), __formatToUint32(4), __formatToUint32(5)})
			So(tr.Len(), ShouldEqual, 5)

			rpv := __formatToUint32(20)
			err := tr.Set(2, rpv)
			So(err, ShouldBeEmpty)
			So(tr.At(2), ShouldEqual, rpv)

			tr.Replace([]uint32{__formatToUint32(1), __formatToUint32(1)})
			So(tr.Len(), ShouldEqual, 5)
			So(tr.At(0), ShouldEqual, tr.At(1))
			So(tr.At(2), ShouldNotEqual, tr.At(0))

			iv1 := __formatToUint32(11)
			err = tr.InsertBefore(0, iv1)
			So(err, ShouldBeNil)
			So(tr.At(0), ShouldEqual, iv1)

			iv2 := __formatToUint32(12)
			...
			pl := __formatToUint32(11)
			tr.PushLeft(pl)
			So(tr.At(0), ShouldEqual, pl)
			pr := __formatToUint32(21)
			tr.PushRight(pr)
			...
			aps := []uint32{__formatToUint32(35), __formatToUint32(40), __formatToUint32(45), __formatToUint32(50)}
			tr.Append(aps...)
			for i := len(aps); i > 0; i-- {
				So(aps[i-1], ShouldEqual, func() uint32 { v, f = tr.PopRight(); So(f, ShouldBeTrue); return v }())
			}
			...
			k := 0
			tr.WalkAsc(func(key int, val uint32) bool {
			...
			k = len(s) - 1
			tr.WalkDesc(func(key int, val uint32) bool {
				So(key, ShouldEqual, k)
				So(val, ShouldEqual, s[k])
				k--
				return true
			...
			So(func() {
				tr.LockFunc(func(array []uint32) {
					return
				})
			}, ShouldNotPanic)

			So(func() {
				tr.RLockFunc(func(array []uint32) {
					return
				})
			}, ShouldNotPanic)
