...
func TestUint8(t *testing.T) {
	Convey("test sync array", t, func() {
		for _, tr := range []*Uint8{NewUint8(), NewSyncUint8()} {
			...
			tr.SetArray([]uint8{__formatToUint8(1), __formatToUint8(1), __formatToUint8(1), __formatToUint8(1), __formatToUint8(1)})
			So(tr.Len(), ShouldEqual, 5)
			tr.SetArray([]uint8{__formatToUint8(1), __formatToUint8(2), __formatToUint8(3), __formatToUint8(4), __formatToUint8(5)})
			So(tr.Len(), ShouldEqual, 5)

			rpv := __formatToUint8(20)
			err := tr.Set(2, rpv)
			So(err, ShouldBeEmpty)
			So(tr.At(2), ShouldEqual, rpv)

			tr.Replace([]uint8{__formatToUint8(1), __formatToUint8(1)})
			So(tr.Len(), ShouldEqual, 5)
			So(tr.At(0), ShouldEqual, tr.At(1))
			So(tr.At(2), ShouldNotEqual, tr.At(0))

			iv1 := __formatToUint8(11)
			err = tr.InsertBefore(0, iv1)
			So(err, ShouldBeNil)
			So(tr.At(0), ShouldEqual, iv1)

			iv2 := __formatToUint8(12)
			...
			pl := __formatToUint8(11)
			tr.PushLeft(pl)
			So(tr.At(0), ShouldEqual, pl)
			pr := __formatToUint8(21)
			tr.PushRight(pr)
			...
			aps := []uint8{__formatToUint8(35), __formatToUint8(40), __formatToUint8(45), __formatToUint8(50)}
			tr.Append(aps...)
			for i := len(aps); i > 0; i-- {
				So(aps[i-1], ShouldEqual, func() uint8 { v, f = tr.PopRight(); So(f, ShouldBeTrue); return v }())
			}
			...
			k := 0
			tr.WalkAsc(func(key int, val uint8) bool {
			...
			k = len(s) - 1
			tr.WalkDesc(func(key int, val uint8) bool {
				So(key, ShouldEqual, k)
				So(val, ShouldEqual, s[k])
				k--
				return true
			...
			So(func() {
				tr.LockFunc(func(array []uint8) {
					return
				})
			}, ShouldNotPanic)

			So(func() {
				tr.RLockFunc(func(array []uint8) {
					return
				})
			}, ShouldNotPanic)
